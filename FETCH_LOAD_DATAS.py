__doc__ = """Ce module contient la définition des variables de chemins de destination ainsi que l'importation des librairies nécessaires à la définition des 2 fonctions suivantes:
	- fetch_seattle_data() : qui sert à télécharger dans un sous-dossier du dossier de travail, 'datasets', le fichier .csv contenant 	le jeu de données complet issu de https://data.seattle.gov/dataset/2016-Building-Energy-Benchmarking/2bpz-gwpy
	- load_seattle_data() : qui sert à charger ce jeu de données dans un DataFrame
"""
...
def fetch_seattle_data(data_path=DATA_PATH, dwnld_url=DOWNLOAD_URL, file_path=FILE_PATH):
...
def load_seattle_data(file_path=FILE_PATH):
